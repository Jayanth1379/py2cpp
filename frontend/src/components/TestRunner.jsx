...
export default function TestRunner({ python, cpp }) {
  ...
  return (
    <div className="pane">
      <h3>Test runner</h3>
      <div className="io">
        <textarea className="editor" placeholder="stdin…" value={stdin} onChange={e=>setStdin(e.target.value)} />
      </div>
      ...
      <div style={{display:"flex", gap:8, padding:8, height:"100%"}}>
        <div className="pane" style={{flex:1}}><h3>Python output</h3><pre className="output">{pyOut}</pre></div>
        <div className="pane" style={{flex:1}}><h3>C++ output</h3><pre className="output">{cppOut}</pre></div>
      </div>
    </div>
  );
}
